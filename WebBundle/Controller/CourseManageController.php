...
class CourseManageController extends BaseController
{
    ...
    public function publishAction(Request $request, $id)
    {
        ...
        // 创建群消息
        $this->createNotification($course);
    ...
    * 创建新课程通知(以群消息的方式)
    */
    protected function createNotification($course)
    {
        // 连接数据库，获取PDO对象
        $pdo = new \PDO('mysql:host=127.0.0.1;dbname=wangxiao', 'root', 'root');
        // 获取数据
        $messageData[':type'] = 'text';
        $messageData[':title'] = $course['title'];
        $messageData[':fromId'] = $this->getCurrentUser()->getId();
        $messageData[':content'] =  $course['title'];
        $messageData[':targetType'] = 'global';
        $messageData[':targetId'] = $course['id'];
        $messageData[':createdTime'] = time();
        $messageData[':published'] = 0;
        $messageData[':sendedTime'] = time();
        // 准备SQL
        $sql = "INSERT INTO `batch_notification` (`type` ,`title`,`fromId`,`content`,`targetType`,`targetId`,`createdTime`,`published`,`sendedTime`)VALUES (:type, :title,:fromId,:content,:targetType,:targetId,:createdTime,:published,:sendedTime)";
    ...
    */
    protected function updateUser()
    {
        ...
        $pdo = new \PDO('mysql:host=127.0.0.1;dbname=wangxiao', 'root', 'root');
        // 准备SQL
        $sql = "UPDATE `user` SET `newNotificationNum`=1 WHERE `newNotificationNum`=:newNotificationNum";
        $stmt = $pdo->prepare($sql);
        // 执行SQL
        $stmt->execute([':newNotificationNum' => 0]);
        // 释放资源
        $pdo = null;
    }
}
