...
class ProgramController extends BaseController
{
    ...
    public function listAction(Request $request,$id)
    {
        ...
        if( $id == (int)$id )
        {
            ...
            //学校列表信息
            $schools = $this->getConsultantService()->searchSchoolList([],[],0,5);
            //获取文章信息
            ...
            if( isset($data['program']) && !empty($data['program']) )
            {
                $where = "s.id={$id} AND p.name LIKE '%{$data['program']}%'";
            }
            else
            {
                $where = "s.id={$id}";
            }
            $programs = $this->getProgramService()->searchPrograms($where,[0,100]);
    ...
    public function detailAction(Request $request,$id)
    {
        ...
        {
            //专业信息
            $info = $this->getProgramService()->getProgram("id={$id}",[0,1]);

            // 学校信息
            $school = $this->getConsultantService()->searchSchoolByid($info['comid']);

            // 案例信息
            $case = $this->getConsultantService()->getCasesBySchoolId($info['comid'],8);

            // echo '<pre>';  print_r($info);print_r($school);exit;

            return $this->render('TopxiaWebBundle:Default:program_detail.html.twig',[
                'info'      =>  $info,
                'school'    =>  $school,
                'case'      =>  $case,
            ]);
